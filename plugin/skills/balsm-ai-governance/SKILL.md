---
name: balsm-ai-governance
description: "Balsm's mandatory AI governance rules for ANY AI/LLM/ML feature in the healthcare platform. Load BEFORE designing, planning, or implementing anything AI: clinical decision support (CDSS), AI chat / chatbot / copilot / assistant, ambient scribing, summarization, triage, recommendations, or any LLM/ML integration. Covers assistive-only clinical AI, human-in-the-loop, no-AI-writes-to-patient-records, PHI and data-boundary rules, AI chat treated as PHI, external-AI-service limits, transparency & explainability, bias & fairness, kill switch, model registry, patient-facing and third-party AI rules, and prompt-injection defense. Also use when citing the AI01-AI16 threat catalog."
risk: safe
source: "mirror of Balsm-Core/AI_GOVERNANCE.md (canonical)"
tags: "[ai, governance, clinical-ai, cdss, phi, safety, healthcare]"
---

# Balsm AI Governance

> **Mirror.** Canonical source is `Balsm-Core/AI_GOVERNANCE.md`. This copy ships in
> the `balsm-ai` plugin so the rules travel to every repo. If the two ever
> diverge, `Balsm-Core/AI_GOVERNANCE.md` wins — refresh this file from it.
> Enforces Constitution Principle X (AI Governance — Assistive Only) and
> Principle I (Patient Safety First, LOCKED).

## Use this skill when

- Designing, planning, or coding ANY AI feature: CDSS, AI chat/chatbot/copilot,
  ambient scribe, summarization, triage, risk scoring, recommendations.
- Integrating any LLM/ML model, or sending data to an external AI service.
- Writing a phase spec that introduces AI (must cite AI01–AI16 threats).
- Reviewing AI code for prompt injection, data leakage, or PHI exposure.

## Non-negotiables (read first)

1. **Assistive only** — AI never makes autonomous clinical decisions, never
   prescribes, never writes to patient records. Human-in-the-loop approval is
   mandatory on every clinical AI workflow.
2. **No AI write access** to patient records — only authorized clinical users.
3. **AI must never bypass rule-based safety checks** (drug interactions, allergy,
   dosage) — those stay deterministic (Principle I).
4. **AI chat containing patient data IS PHI** — encrypt at rest, audit-log,
   RBAC-scope to the invoking user, redact secrets, honor retention + deletion.
5. **No PHI to external AI in identifiable form** — de-identify; log all external
   calls; entities can opt out entirely without platform degradation.
6. **Kill switch + model registry** — admins disable any/all AI instantly; every
   active model is registered with purpose, inputs, limitations.
7. **Bias evaluated pre-deployment** and ongoing; no AI for eligibility/access/
   prioritization that could discriminate.
8. **Platform works fully without AI** — enhancement layer, never a dependency.
9. **Cite AI01–AI16** from `Balsm-Core/SYSTEM_THREAT_MODEL.md` in any AI phase
   spec and confirm all `BLOCKING` mitigations are covered.
...
See `Balsm-Core/SYSTEM_THREAT_MODEL.md` § AI/ML-Specific Threats for the AI/ML
threat catalog (AI01–AI16) and mitigations. The same document contains the
platform-wide cybersecurity and application-security threats that AI features
inherit.

Every phase spec that introduces an AI feature must reference the applicable
threat IDs from that document and confirm all `BLOCKING` mitigations are covered
